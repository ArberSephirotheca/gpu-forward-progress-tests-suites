        ...
        for file_name in files:
            if file_name.endswith('.comp'):
                ...
                if android:
                    ...
                    run_cmd = (
                        f"timeout -k 1 15 adb shell 'cd /data/local/tmp ; ./amber_ndk "
                        f"{amber_build_flags} {amber_file_path.name}' >> temp_results.txt"
                    )

                for i in range(int(num_iter)):
                    log_print(f"running test: {amber_file_path}")
                    log_print(run_cmd)
                    os.system(run_cmd)
